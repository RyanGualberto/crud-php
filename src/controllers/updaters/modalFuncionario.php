...
$id = $_GET['id'];
$select = $conn->prepare("SELECT * FROM mysystem.tb_funcionario where cd_funcionario=$id");
$select->execute();
$row = $select->fetch();
     ...
     <div class='modal-dialog'>
         <div class='modal-content'>
             <div class='modal-header'>
                 ...
                 <button type='button' onclick='window.location.href=`../../views/consultaFuncionario.php`'  class='btn-close' data-bs-dismiss='modal' aria-label='Close'></button>
             </div>
             <form method='POST'>
                 <div class='mb-3'>
...
if (!empty($_POST)) {
    ...
    try {
        ...
        $stmt = $conn->prepare("
    USE mySystem;
     UPDATE tb_funcionario SET nm_primeiro = :primeironm, nm_sobrenome= :sobrenome, nr_cpf = :cpf, nr_rg = :rg, nr_cep = :cep, nr_endereco = :endereco, nr_celular = :tel, nm_email = :email, id_genero = :idgender WHERE cd_funcionario = :cd");


        $stmt->bindParam(':cd', $row['cd_funcionario']);
        ...
        echo "<script>window.location.href = '../../views/consultarFuncionario.php';</script>";
